...
export function Navbar() {
    ...
    return (
        <header role="banner" className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-black/70 backdrop-blur-xl border-b border-white/10 py-4 shadow-2xl shadow-black/50' : 'bg-transparent py-6'}`}>
            <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 flex justify-between items-center">
                ...
                {/* Desktop Nav */}
                <nav aria-label="Main navigation" className="hidden md:flex gap-10 items-center ml-16">
                    {navLinks.map((link) => {
                        const isActive = link.href.startsWith('/') && location.pathname.startsWith(link.href);
                ...
                {/* Mobile Menu Button */}
                <button aria-label={mobileMenuOpen ? 'Close menu' : 'Open menu'} className="md:hidden text-white z-50 hover:text-accent transition-colors" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                        ...
                        {navLinks.map((link, i) => {
                            ...
                            return link.href.startsWith('/') ? (
                                ...
                                >
                                    <Link
                                        to={link.href}
                                        className={`text-4xl font-display font-medium hover:scale-105 transition-all ${isActive ? 'text-accent' : 'text-white/80 hover:text-white'}`}
